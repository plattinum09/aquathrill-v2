...
export async function availability(request:NextRequest){
  ...
  const counts=(await query<any>(
    selected==="all"
      ? "SELECT boat_type,booking_date::text,time_slot,COUNT(*)::int cnt FROM bookings WHERE booking_date BETWEEN $1::date AND $2::date AND status='confirmed' GROUP BY boat_type,booking_date,time_slot"
      : "SELECT boat_type,booking_date::text,time_slot,COUNT(*)::int cnt FROM bookings WHERE boat_type=$1 AND booking_date BETWEEN $2::date AND $3::date AND status='confirmed' GROUP BY boat_type,booking_date,time_slot",
    selected==="all"?[start,end]:[selected,start,end]
  )).rows;
  ...
  return publicJson({month,year,calendar,details,pastDates},200,"no-store, no-cache, must-revalidate");
}
