...
function normalizeFolders(folders:any[]){
  return (Array.isArray(folders)?folders:[]).map((folder:any,index:number)=>({
    ...folder,
    id:String(folder?.id||`folder_${index}`),
    name:String(folder?.name||folder?.title||`อัลบั้ม ${index+1}`),
    created_at:String(folder?.created_at||folder?.createdAt||folder?.date||new Date().toISOString()),
  }));
}
function normalizeItems(items:any[]){
  return (Array.isArray(items)?items:[]).map((item:any,index:number)=>({
    ...
    event_date:String(item?.event_date||item?.date||item?.uploaded_at||new Date().toISOString()).slice(0,10),
...
export async function gallery(request:NextRequest){
  ...
  if(request.method==="POST"){
    ...
    if(form.get("action")==="replace_image"){const id=String(form.get("id")||"");const item=items.find((x:any)=>x.id===id);if(!item)return json({error:"Item not found"},404);if(item.type==="youtube")return json({error:"ไม่สามารถเปลี่ยนรูปของวิดีโอ YouTube ได้"},400);const file=form.get("image");if(!(file instanceof File))return json({error:"No file uploaded"},400);if(file.size>10*1024*1024)return json({error:"ไฟล์ใหญ่เกินไป สูงสุด 10MB"},400);if(!["image/jpeg","image/png","image/webp","image/gif"].includes(file.type))return json({error:"ไฟล์ไม่ถูกต้อง"},400);const oldTarget=deleteTarget(item);const blob=await putGalleryImage(file);item.src=blob.url;item.blob_pathname=blob.pathname;item.blob_access=blob.access;item.uploaded_at=new Date().toISOString();item.source="admin_upload";await save("gallery_items",items);if(oldTarget&&oldTarget.includes("gallery/"))await del(oldTarget).catch(console.warn);return json({success:true,item});}
